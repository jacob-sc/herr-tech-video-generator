...
export default async function handler(req, res) {
  ...
  if (!scene.imageFile) return res.status(400).json({ error: 'Kein Bild vorhanden zum Freigeben' });

  const updatedScenes = [...project.scenes];
  updatedScenes[sceneIdx] = { ...scene, imageApproved: approved };
  updateProject(id, { scenes: updatedScenes });

  return res.status(200).json({ ok: true, sceneId: sceneIdx, imageApproved: approved });
}
